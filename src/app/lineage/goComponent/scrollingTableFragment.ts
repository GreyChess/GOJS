...
export class ScrollingTableFragment{
  static init(){
...
// you'll want to make sure each one has a unique name.
    go.GraphObject.defineBuilder('ScrollingTable', function (args) {
      ...
      function incrTableIndex(obj, i) {
        ...
        const table = obj.panel.panel.panel.findObject(tablename);
        ...
        if (table.topIndex !== idx) {
          ...
          table.topIndex = idx;
          let node = table.part;  // may need to reroute links if the table contains any ports
      ...
      function updateScrollBar(table) {
        let bar = table.panel.elt(1);  // the scrollbar is a sibling of the table
        if (!bar) return;
        let idx = table.topIndex;

        let up = bar.findObject('UP');
        if (up) up.opacity = (idx > 0) ? 1.0 : 0.3;

        let down = bar.findObject('DOWN');
        if (down) down.opacity = (idx < table.rowCount - 1) ? 1.0 : 0.3;

        let tabh = bar.actualBounds.height;
        ...
        let numVisibleRows = Math.max(1, Math.ceil(tabh / rowh) - 1);
        let needed = idx > 0 || idx + numVisibleRows <= table.rowCount;
      ...
      return $(go.Panel, 'Table',
        ...
        $(go.Shape, "RoundedRectangle", {stroke: "white", fill: "rgba(0,0,0,0)"})
      );
    });
